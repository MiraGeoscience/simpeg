...
class Simulation3DIntegral(BasePFSimulation):
    ...
    def fields(self, model):
        ...
        if self.store_sensitivities is None:
            ...
            fields = np.asarray(
                self.Jmatrix @ self.chi.astype(self.sensitivity_dtype, copy=False)
            )
    ...
    def Jvec(self, m, v, f=None):
        ...
        Jvec = self.Jmatrix @ dmu_dm_v.astype(self.sensitivity_dtype, copy=False)
    ...
    def Jtvec(self, m, v, f=None):
        ...
        Jtvec = self.Jmatrix.T @ v.astype(self.sensitivity_dtype, copy=False)
    ...
    def ampDeriv(self):
        if getattr(self, "_ampDeriv", None) is None:
            fields = np.asarray(
                self.Jmatrix.dot(self.chi).astype(self.sensitivity_dtype, copy=False)
            )
