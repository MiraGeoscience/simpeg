...
def fields(self, m=None, return_Ainv=False):
    ...
    for tInd, dt in enumerate(self.time_steps):
        ...
        if np.abs(self.survey.source_list[0].waveform.eval(self.times[tInd + 1])):
...
def dask_getSourceTerm(self, tInd):
    ...
    sources_eval = dask.compute(block_compute)[0]

    s_m, s_e = [], []
    for block in sources_eval:
        if block[0]:
            s_m.append(block[0])
            s_e.append(block[1])
...
def compute_J(self, f=None, Ainv=None):
    ...
    if self.store_sensitivities == "disk":
        self.J_initializer = zarr.open(
            self.sensitivity_path + "J_initializer.zarr",
            mode="w",
    ...
    for tInd, dt in tqdm(zip(reversed(range(self.nT)), reversed(self.time_steps))):
        ...
        for isrc, elem in enumerate(dask.compute(source_blocks)[0]):
            field_derivs_t[isrc] = elem
    ...
    if self.store_sensitivities == "disk":
        del Jmatrix
        return array.from_zarr(self.sensitivity_path + "J.zarr")
    else:
